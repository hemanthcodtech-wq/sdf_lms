...
const PublicNavbar = () => {
  ...
  return (
    <motion.nav
      ...
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 bg-white ${isScrolled ? 'shadow-md border-b border-gray-100' : 'shadow-sm'}`}
    >
      {/* Main Navbar Row */}
      <div className="h-18 md:h-20 flex items-center w-full border-b border-gray-50 md:border-b-0">
      ...
      {/* Mobile Drawer Menu (Vertically below navbar, 100% width, clear buttons, no cutoffs) */}
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="md:hidden w-full bg-white border-t border-gray-100 shadow-2xl overflow-hidden"
          >
            <div className="px-4 py-4 flex flex-col space-y-2 max-w-full">
              {navLinks.map((link) => (
                <Link
                  key={link.path}
                  to={link.path}
                  onClick={() => setIsMobileMenuOpen(false)}
                  className={`flex items-center px-4 py-3 rounded-xl font-outfit font-semibold text-base transition-colors ${
                    isActive(link.path) 
              ...
              <div className="mt-3 pt-3 border-t border-gray-100 flex flex-col space-y-2.5">
                {token ? (
                  <Link
                    to={user?.role === 'admin' ? '/admin/dashboard' : '/dashboard'}
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="w-full text-center py-3 rounded-xl bg-brand-green text-white font-bold font-outfit text-sm shadow-md hover:bg-brand-green-dark"
                  >
                    {t('nav_dashboard')}
                  </Link>
                ) : (
                  <>
                    <Link 
                      to="/login" 
                      onClick={() => setIsMobileMenuOpen(false)}
                      className="w-full text-center py-3 rounded-xl border-2 border-brand-green text-brand-green font-bold font-outfit text-sm hover:bg-brand-green/5"
                    >
                      {t('nav_login')}
                    </Link>
                    <Link 
